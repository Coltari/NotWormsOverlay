...
func get_token() -> void:
	...
	OS.shell_open("https://id.twitch.tv/oauth2/authorize?response_type=code&client_id=" + client_id +"&redirect_uri="+Events.redirectURL+"&scope=" + scope)
	var port = Events.redirectURL.substr(Events.redirectURL.find(":",0),Events.redirectURL.length()-Events.redirectURL.find(":",0))
	server.listen(7170)
	...
	while(peer.get_status() == peer.STATUS_CONNECTED):
		...
		if (peer.get_available_bytes() > 0):
			...
			if (data.has("error")):
				...
				var request : HTTPRequest = HTTPRequest.new()
				add_child(request)
				request.request("https://id.twitch.tv/oauth2/token", [USER_AGENT, "Content-Type: application/x-www-form-urlencoded"], HTTPClient.METHOD_POST, "client_id=" + client_id + "&client_secret=" + client_secret + "&code=" + data["code"] + "&grant_type=authorization_code&redirect_uri="+Events.redirectURL)
